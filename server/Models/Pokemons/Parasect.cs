...
public class Parasect : PokemonMaster
{
    ...
    public Parasect(float HP, string nickname, string ownerId, int exp)
    : base("Parasect", "Bug/Grass", HP, 95, 80, 60, 80, 30, ownerId, 24, "Effect Spore")
    {
        Nickname = nickname;
        Experience = exp;
        SkillPool = "Scratch, Stun Spore, Leech Life, Spore, Slash, Growth, SolarBeam, Toxic, Body Slam, Take Down, Double-Edge, Hyper Beam, Mimic, Double Team, Reflect, Bide, Rest, Substitute, Cut";
    ...
    : base("Parasect", "Bug/Grass", 100, 95, 80, 60, 80, 30, paras.OwnerId ?? "Unknown", 24, "Effect Spore")
    {
        Id = paras.Id;
        Level = 1;
